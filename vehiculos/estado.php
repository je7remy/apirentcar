...
if ($jwt) {
    // Si el JWT es válido, actualizar el estado del vehículo
    try {
        // Establecer los valores de propiedad del vehículo
        $vehiculo->idvehiculo = $data->idvehiculo;
        $vehiculo->estado = $data->estado;

        // Actualizar el estado del vehículo
        $result = $vehiculo->updateEstado();
        ...
        if ($result) {
            ...
            $json = array(
                "status"    => "true",
                "errCode"   => "00",
                "msg"       => "Error al actualizar el estado del vehículo"
            );
            echo json_encode($json);	
        }
    } catch (Exception $e) {
